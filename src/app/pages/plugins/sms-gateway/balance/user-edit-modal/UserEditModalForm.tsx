...
const UserEditModalForm: FC<Props> = ({user, isUserLoading}) => {
  ...
  const editUserSchema = Yup.object().shape({
    number_of_sms: Yup.number()
      .min(1, 'Number of SMS must be al least 1!')
    ...
    total_price: Yup.number().min(1, 'Pricing must be al least 1!').required('Pricing is required'),
    remarks: Yup.string(),
  ...
  const [userForEdit] = useState<Bundle>({
    ...user,
    number_of_sms: user?.number_of_sms || 1,
    account_type: user?.account_type || 'Non-Masking',
    pricing: user?.pricing || 1,
    total_price: user?.total_price || 1,

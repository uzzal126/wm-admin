...
const getUserById = (id: ID): Promise<Coupon | undefined> => {
  return axios
    .get(`${GET_COUPON_DETAILS}/${id}`)
    .then((response: AxiosResponse<Response<Coupon>>) => response.data)
    .then((response: Response<Coupon>) => response.data)
}

const createCoupon = (user: Coupon): Promise<Coupon | undefined> => {
  return axios
    .post(POST_ADD_COUPON, user)
    .then((response: AxiosResponse<Response<Coupon>>) => response.data)
    .then((response: any) => {
      if (response?.success) {
...
const updateCoupon = (user: Coupon): Promise<Coupon | undefined> => {
  return axios
    .put(`${POST_UPDATE_COUPON}/${user?.id}`, user)
    .then((response: AxiosResponse<Response<Coupon>>) => response.data)
    .then((response: any) => {
      if (response?.success) {
...
const deleteCoupon = (userId: ID): Promise<void> => {
  // return axios.delete(`${USER_URL}/${userId}`).then(() => { })
  return axios({
    method: 'post',
    url: `${POST_DELETE_COUPON}`,
    data: {
      coupon_id: userId,
    },
  }).then(() => {})

...
import {GET_CAMPAIGN_PRODUCT, GET_SETTING, UPDATE_STATUS} from '../../../../constants/api.constants'
            ...
            campData.map((cp, i) => (
              <Fragment key={'cps' + i}>
                <tr>
                  ...
                  <td>{cp.name}</td>
                  <td>{moment.unix(cp?.start_at).format('LLL')}</td>
                  <td>{moment.unix(cp?.end_at).format('LLL')}</td>
                  <td>
                    <Link
                      state={cp}
                      to={`/marketing/campaign/${cp.group_slug}/products/${cp.id}`}
                ...
                {products &&
                  ...
                  products.data.map((csp, j) => (
                    <ShowProducts key={j} indx={j} item={csp} store_info={storeData?.store_info} />
                  ))}
              </Fragment>
            ))
          ) : (

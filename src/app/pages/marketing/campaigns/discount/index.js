...
const DiscountProduct = ({camp}) => {
            ...
            discount:
              typeof item.campaigns === 'string'
                ? JSON.parse(item.campaigns).length > 0
                  ? JSON.parse(item.campaigns)[0].discount
                  : 0
                : item.discount.campaign_discount_amount,
          }
          pdid.push(newId)
        }
      })
  ...
  const handleBulkDiuscounts = (val) => {
    if (pdIds && pdIds.length > 0) {
      const pdid = [...pdIds]
      pdid.map((nid, i) => {
        const product = products?.filter((e) => e?.id === nid?.id)[0]
        pdid[i] = {
          ...nid,
          discount:
            product && product?.price?.min
              ? val > product?.price?.min
                ? product?.price?.min
                : val
              : 0,
        }
  ...
  const handleDiscounts = (val, checkId) => {
    if (pdIds && pdIds.length > 0) {
      const pdid = [...pdIds]
      pdid.map((nid, i) => {
        if (nid.id === checkId)
                            ...
                            <tr key={i}>
                              ...
                              <td className=''>
                                <div className='text-muted fs-7 fw-bolder'>
                                  {prepareCategories(prod?.categories || []).join(', ')}
                                </div>
                              </td>
                              <td className=''>
                                <div className='text-muted fs-7 fw-bolder'>
                                  {prod?.inventory?.avilable === null
                                    ? 0
                                    : prod?.inventory?.avilable}
                                </div>
                              </td>
                              <td className=''>
                                <div className='text-muted fs-7 fw-bolder'>
                              ...
                              <td className=''>
                                <div className='text-muted fs-7 fw-bolder'>
                                  {prod.discount.general_discount_amount}
                                </div>
                              </td>
                              <td className='w-100px'>
                                <div className='input-group input-group-sm w-150px'>
                                  <input
                                    ...
                                    onChange={(e) => handleDiscounts(e.target.value, prod.id)}
                                    aria-label='Bulk Discount'

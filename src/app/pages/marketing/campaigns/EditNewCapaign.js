...
const EditNewCapaign = ({edit, setEdit, cmp, setData}) => {
  ...
  const [start_at, setstart_at] = useState(
    moment.unix(edit.start_at).format('YYYY-MM-DD hh:mm:ss') ||
      moment(new Date()).format('YYYY-MM-DD hh:mm:ss')
  )
  const [end_at, setend_at] = useState(
    moment.unix(edit.end_at).format('YYYY-MM-DD hh:mm:ss') ||
      moment(new Date()).format('YYYY-MM-DD hh:mm:ss')
  )
  ...
  const handleSubmitForm = async (post) => {
    ...
    } else if (calculateUtf8Length(post?.slug) > 500) {
      toast.error('Campaign title can not exceed 500 utf8 characters!', {
        position: 'top-right',
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,

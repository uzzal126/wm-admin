...
const GET_TEMPLATES = `${BASE_URL}/sms/templates`
...
const getUsers = (query: any): Promise<TemplatesQueryResponse> => {
  return axios({
    method: 'get',
    url: `${GET_TEMPLATES}?${query}`,
  }).then((d: AxiosResponse<TemplatesQueryResponse>) => d.data)
}

const getUserById = (id: ID): Promise<Template | undefined> => {
  return axios
    .get(`${GET_TEMPLATE}/${id}`)
...
const createUser = (user: Template): Promise<any> => {
  return axios.post(USER_URL, user).then((response: AxiosResponse<any>) => response.data)
}

const updateUser = (user: Template): Promise<any> => {
  return axios
    .put(`${GET_TEMPLATE}/${user.id}`, user)
...
export {createUser, deleteSelectedUsers, deleteUser, getUserById, getUsers, restoreUser, updateUser}

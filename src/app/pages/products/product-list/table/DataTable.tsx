...
import {RenderAttribute} from './column/RenderAttribute'
...
const ProdutDataTable = () => {
  ...
  const {selectedAttribute, selected} = useListView()
              ...
              rows.map((row: Row<Product>, i) => (
                <React.Fragment key={i}>
                  <>
                    ...
                    {row.original && row.original?.prod_id === selectedAttribute && (
                      <RenderAttribute pid={selectedAttribute} />
                    )}
                  </>
                </React.Fragment>
              ))

...
import {BASE_URL, PAGE_URL} from '../../../../../constants/api.constants'
...
const updateUser = (user: TableModal): Promise<TableModal | undefined> => {
  return axios
    .post(`${PAGE_URL}/${user.id}`, user)
    .then((response: AxiosResponse<Response<TableModal>>) => response.data)
    .then((response: Response<TableModal>) => response.data)
}

const deleteUser = (userId: ID): Promise<void> => {
  return axios.delete(`${PAGE_URL}/${userId}`).then(() => {})
}

const deleteSelectedUsers = (userIds: Array<ID>): Promise<void> => {
  const requests = userIds.map((id) => axios.delete(`${PAGE_URL}/${id}`))

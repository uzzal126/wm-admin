...
const QueryResponseContext = createResponseContext<Job>(initialQueryResponse)
const QueryResponseProvider: FC = ({children}: any) => {
  ...
  const { isFetching, refetch, data: response, } = useQuery({
    queryKey: [`${QUERIES.USERS_LIST + state?.slug}-${query}`],
    queryFn: async () => {
      const data = state?.slug ? getUsers(state?.slug, query) : []
      // const { data } = await dispatch(jobApi.endpoints.jobPost.initiate(query))

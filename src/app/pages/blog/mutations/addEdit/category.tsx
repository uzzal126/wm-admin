...
import { FormField } from '../../../products/components/FormField'
...
const CategoryPopup = ({setFieldValue, values}: Props) => {
  ...
  const saveNewCategory = async (event: any) => {
    event.preventDefault();

    if (isUndefinedNullOrWhiteSpaceString(name)) { return; }
    ...
    try {
      ...
      if (!res?.success) { throw new Error(res?.message ?? 'An error occurred while saving new category'); }

      setNewCategory('');
      toast.success(res?.message ?? 'Successfully saved new category');
      handleClose()
    } catch (error) {
      // console.error('An error occurred while saving new category.', error);

      const message: string = (error as any)?.data?.message
        ?? (error as any)?.message
        ?? 'An error occurred while saving new category';
  ...
  return (
    <>
      ...
      <Modal show={showCatModal} onHide={handleClose} centered>
        ...
        <Modal.Body>
          <form className='row' autoComplete='off' onSubmit={saveNewCategory}>
            <div className='col'>

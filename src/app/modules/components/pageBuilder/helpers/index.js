...
import {COLUMN, COMPONENT, ROW} from '../data/constants'
  ...
  switch (splitDropZonePath.length) {
    case 1: {
      newLayoutStructure = {
        type: ROW,
        id: generateUniqueId(),
        children: [{type: COLUMN, id: generateUniqueId(), children: [item]}],
      }
      break
    }
    case 2: {
...
export const layoutUpdateComponent = (layout, id, value) => {
  layout &&
    layout.length > 0 &&
    layout.map((row, ri) => {
      row.children &&
        row.children.length > 0 &&
        row.children.map((column, ci) => {
          column.children &&
            column.children.length > 0 &&
            column.children.map((item, i) => {
              if (item.id === id) {
                layout[ri].children[ci].children[i] = value
              }
            })
        })
    })
